...
import { sendSignupVerificationCode, signup } from "@/app/utils/api";
import { getGsmEmailErrorMessage, isValidPassword } from "@/app/utils/authValidation";
import { getAuthErrorMessage, getSignupError } from "@/app/utils/authErrorMessages";
...
export default function SignupPage() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    if (emailError) return setErrors((current) => ({ ...current, email: emailError }));
  ...
  const sendCode = async () => {
    ...
    try {
      ...
      setErrors((current) => ({
        ...current,
        email: getAuthErrorMessage(caught, "인증코드를 발송하지 못했습니다. 잠시 후 다시 시도해주세요."),
      }));
    } finally {
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center">
      <form noValidate onSubmit={submit} className="flex flex-col items-center">
        ...
        {submitError ? <p role="alert" className="mt-1 w-[600px] text-right text-[15px] text-[#D61E1E]">{submitError}</p> : null}
