...
type FieldError = { field: AuthErrorField; message: string };
...
const asError = (error: unknown): ErrorLike => typeof error === "object" && error !== null ? error as ErrorLike : {};
...
export const getAuthErrorMessage = (error: unknown, fallback: string) => {
  ...
  if (typeof current.status === "number" && current.status >= 500) return "서버 오류가 발생했습니다. 잠시 후 다시 시도해주세요.";
  return current.message ? messages[current.message] || fallback : fallback;
};

const rawMessage = (error: unknown) => asError(error).message || "";

export const getSignupError = (error: unknown): FieldError => {
  const raw = rawMessage(error);
  const message = getAuthErrorMessage(error, "회원가입에 실패했습니다. 잠시 후 다시 시도해주세요.");
...
export const getPasswordResetError = (error: unknown): FieldError => {
  const raw = rawMessage(error);
  const message = getAuthErrorMessage(error, "비밀번호를 변경하지 못했습니다. 잠시 후 다시 시도해주세요.");
  if (raw === "Invalid verification code." || raw === "Verification code has expired.") {
    return { field: "verificationCode", message };
  }
  if (raw === "이메일은 학교계정을 사용해야합니다.") return { field: "email", message };
  return { field: "form", message };
};
