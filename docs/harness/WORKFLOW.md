...
1. 사용자 요청을 전체 FSD structural migration인지 Student 디자인 재구성인지 분류한다.
...
## Student design rebuild

사용자가 새 디자인을 전달한 뒤에만 진행한다. 기존 화면 코드를 그대로 옮기는 것이 목표가 아니라, 기존 사용자 기능과 API/session contract를 재사용하면서 FSD 기준으로 Student UI/함수/state/component를 새 디자인에 맞춰 재구성한다.

이 단계에서 Teacher/Admin 보호 경로는 수정하지 않는다.

## Teacher/Admin structural migration

Teacher/Admin structural migration은 `refactor/teacher-fsd*` branch 또는 명시적 migration mode에서만 한다. 구현 전에 현재 동작에 대한 characterization/regression을 추가하고 RED/GREEN이 아니라 **기존 동작을 고정하는 baseline test**임을 명확히 기록한다.

Teacher/Admin에서 redesign이나 business rewrite가 필요해 보여도 Student 디자인 재구성 작업에 섞지 않는다.
...
- `refactor/teacher-fsd*`: Teacher/Admin behavior-preserving FSD migration 전용
