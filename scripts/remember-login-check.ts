...
const entityLifecycle = readFileSync(resolve(process.cwd(), "src/fsd/entities/user/model/lifecycle.ts"), "utf8");
...
assert.match(entityLifecycle, /clearSession[\s\S]*backfillRememberLoginEmail\(session\.email\)[\s\S]*clearStoredSession/);
...
const rememberBlock = login.match(/<label[\s\S]*?아이디 저장[\s\S]*?<\/label>/)?.[0] || "";
