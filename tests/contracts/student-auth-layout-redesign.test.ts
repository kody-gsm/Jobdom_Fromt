...
assert.match(layout, /flex min-h-screen items-center justify-center/);
assert.match(layout, /max-w-\[520px\]/);
assert.match(layout, /src="\/JobdamIcon\.svg"/);
assert.match(layout, /className="h-auto w-\[132px\]"/);
assert.doesNotMatch(layout, /lg:grid-cols-\[42%_58%\]|brightness-0 invert/);
assert.doesNotMatch(layout, /JOBDAM FOR GSM|취업과 상담을 한 곳에서/);
...
console.log("student auth centered layout contract passed");
