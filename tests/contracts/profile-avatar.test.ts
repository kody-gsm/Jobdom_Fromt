...
assert.equal(getProfileAvatarStorageKey("2401"), "jobdam.profile-avatar.2401");
assert.equal(getProfileAvatarUserKey({ email: " USER@GSM.HS.KR " }), "user@gsm.hs.kr");
assert.equal(getProfileAvatarUserKey({ studentId: "2401" }), "2401");
...
assert.match(source, /PROFILE_AVATAR_CHANGED_EVENT/);
