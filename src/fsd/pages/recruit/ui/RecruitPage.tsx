    ...
    <div className="min-h-dvh bg-[#F4F6F8] text-[#13233A]" style={{ fontFamily: '"Pretendard Variable", sans-serif' }}>
      ...
      <main className="mx-auto w-full max-w-[1180px] px-6 py-10 lg:px-10 lg:py-12">
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <Link href="/forms" className="inline-flex min-h-11 items-center rounded-lg px-3 text-sm font-bold text-[#02A94A] transition-colors hover:bg-[#E9FAF0]">
            신청 폼 보기
          </Link>
        </div>
...
const RecruitCard = ({ item }: { item: Recruit }) => (
  <ContentCard className="flex min-h-[330px] flex-col p-7">
    ...
    <h2 className="mt-6 break-keep text-2xl font-bold tracking-[-0.02em] text-[#13233A]">{item.companyName || "회사명 확인 중"}</h2>
    <p className="mt-3 line-clamp-4 flex-1 whitespace-pre-line break-keep text-sm leading-7 text-[#667281]">{item.summary || "공고 요약이 없습니다."}</p>
    <dl className="mt-6 space-y-2 border-t border-[#E8EBEF] pt-5 text-sm">
      <div className="flex justify-between gap-3"><dt className="text-[#8A95A3]">지원 마감</dt><dd className="font-semibold text-[#4E5B6B]">{item.deadline || "별도 확인"}</dd></div>
      <div className="flex justify-between gap-3"><dt className="text-[#8A95A3]">면접 일정</dt><dd className="font-semibold text-[#4E5B6B]">{item.interviewDate || "별도 확인"}</dd></div>
    </dl>
    <Link href={`/recruit/${item.id}`} className="mt-6 inline-flex h-12 items-center justify-center rounded-xl bg-[#10243E] px-5 text-sm font-bold text-white hover:bg-[#1B3555]">공고 상세 보기</Link>
  </ContentCard>
);
